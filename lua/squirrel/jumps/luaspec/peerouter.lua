...
local function goto_cond_peer(winid, node, itype)
  if itype == "if_statement" then
    if at_node_end() then return nuts.goto_node_beginning(winid, node) end
    for i = 0, node:named_child_count() - 1 do
      local child = node:named_child(i)
      if is_cond_node(child:type()) then return nuts.goto_node_beginning(winid, child) end
    end
    return nuts.goto_node_end(winid, node)
  end

  if itype == "elseif_statement" then
    local sib = node:next_named_sibling()
    while sib ~= nil do
      if is_cond_node(sib:type()) then return nuts.goto_node_beginning(winid, sib) end
      sib = sib:next_named_sibling()
    end
    return nuts.goto_node_end(winid, assert(node:parent()))
  end

  if itype == "else_statement" then return nuts.goto_node_end(winid, assert(node:parent())) end
